"""
Algoritmo para gestionar las mesas de Casa Paco
"""
...
# Cada grupo de comensales en la cola
class Grupo:
...
# Mostrar la información del segundo que es, las colas y las mesas
def mostrar_info(mesas: dict, colas: list, segundos: int):

    # Segundo
    print(f"\n\nSegundo: {segundos}")

    # Cola de personas atendidas
    print("\nCola atendida: ")
    if colas[1]:
        for grupo in colas[1]:

            # Imprimir información por cada grupo de personas en la cola
            print(f"Posicion: {grupo.posicion}\t\tNumero de personas: "
                  f"{grupo.comensales}\t\tTiempo para comer: {grupo.tiempo}")
    else:
        print("Vacia")

    # Cola de personas sin atender
    print("\nCola sin atender: ")
    if colas[0]:
    ...
    for mesa in mesas:
        ...
        # Imprimir si las mesas vacías o ocupadas
        if mesas[mesa].tiempo_restante <= 0:
...
def main(mesas: dict, colas: list, tiempo: int = 300):
    ...
    # Mientras haya gente en la cola
    while colas[0] or colas[1]:

        # Calcular las mesas disponibles
        mesas_libres = mesas_disponibles(mesas)

        # Imprimir toda la información
        mostrar_info(mesas, colas, segundos)

        # Primero se intenta sentar a la gente que ya ha sido atendida y está esperando
        if colas[1]:
            for posicion, grupo in enumerate(colas[1]):

                # Por posición en la fila se comprueba si hay una mesa con
                # la misma capacidad que número de personas en el grupo
                for id, mesa in enumerate(mesas_libres):  # Por cada mesa en las mesas libres
                    if mesa.capacidad == grupo.comensales:  # Si la mesa tiene capacidad perfecta
                        mesa.tiempo_restante = grupo.tiempo  # Añadir el tiempo que van a tardar en comer
                        colas[1].pop(posicion)  # Quitar al grupo de la cola
                        mesas_libres.pop(id)  # Quitar la mesa de las mesas disponibles
                        break  # Pasar al siguiente grupo

                # Si no hay una mesa con la misma capacidad se comprueba si hay una mesa con uno más de capacidad
                else:
                    for id, mesa in enumerate(mesas_libres):  # Por cada mesa en las mesas libres
                        if mesa.capacidad == grupo.comensales + 1:  # Si la mesa tiene capacidad adecuada
                            mesa.tiempo_restante = grupo.tiempo  # Añadir el tiempo que van a tardar en comer
                            colas[1].pop(posicion)  # Quitar al grupo de la cola
                            mesas_libres.pop(id)  # Quitar la mesa de las mesas disponibles
                            break  # Pasar al siguiente grupo

        # Después se intenta sentar a la gente que todavía no ha sido atendida
        if colas[0]:
            for posicion, grupo in enumerate(colas[0]):
                ...
                # la misma capacidad que número de personas en el grupo
                for id, mesa in enumerate(mesas_libres):  # Por cada mesa en las mesas libres
                    if mesa.capacidad == grupo.comensales:  # Si la mesa tiene capacidad adecuada
                        mesa.tiempo_restante = grupo.tiempo  # Añadir el tiempo que van a tardar en comer
                        colas[0].pop(posicion)  # Quitar al grupo de la cola
                        mesas_libres.pop(id)  # Quitar la mesa de las mesas disponibles
                        break  # Pasar al siguiente grupo
